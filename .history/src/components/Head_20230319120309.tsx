import React from 'react'

const Head = () => {
  return (
    <div>
        <img src="data:image/png;base64," alt="menu"/>
        <img src="https://showponycreative.com.au/showpony_content/uploads/2020/02/302-3020719_youtube-music-logo-png-transparent-background-youtube-logo.jpg" alt="Youtube" />
    </div>
  )
}

export default Head